...
public interface MemberService {
	...
	// 아이디 조회
	Member findMemberById(@Param("mId") String id);
	
	// 로그인
	Member login(@Param("mId") String id, @Param("mPassword") String password);
	
	// 회원가입 (1이면 ㅇㅋ 0이면 ㄴㄴ)
	int save(Member member);

	// 아이디 중복검사(안씀)
	Boolean isDuplicateId(@Param("mId") String id);
	
	// 회원 탈퇴
	int delete(@Param("mNo") int no);
